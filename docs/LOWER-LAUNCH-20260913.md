...
## Scope
Navy Studio; white, aligned four-column process (two at tablet, one on mobile); light #F6F8FA Contact and restored white form; existing compact navy footer. Frozen product chapters and case studies unchanged. Exact founder copy preserved. Privacy provider disclosure now Resend. Machine index includes exact hero canon. Fonts retain their files and use bounded cache freshness; hashed assets remain immutable.
...
Turnstile mode: existing dedicated `1118 Contact Form — Production`, Managed, approved hosts `1118.io`, `www.1118.io`. Widget identity/settings are historical documented evidence; live Cloudflare account configuration is not verified in this run. Production accepts only real server-configured keys, rejects official test keys/test mode. Unknown environments reject. Preview/development are forcibly mapped to Cloudflare's official public test pair and require the official testing metadata in Siteverify. Preview never uses real Production credentials.
