...
type ContactDraft = { name: string; email: string; company: string; stage: string; links: string; message: string; website: string };
type Errors = Partial<Record<keyof ContactDraft, string>>;
const emptyDraft: ContactDraft = { name: '', email: '', company: '', stage: '', links: '', message: '', website: '' };
...
function ActiveContactPanel() {
  ...
  const update = (field: keyof ContactDraft, value: string) => {
    ...
    if (status !== 'sending') { setStatus('idle'); setNotice(''); if (draft[field].trim() !== value.trim()) submission.current = ''; }
  };
  const submit = async (event: FormEvent<HTMLFormElement>) => {
    ...
    if (draft.links.trim()) { try { const url = new URL(draft.links.trim()); if (!['https:', 'http:'].includes(url.protocol) || url.username || url.password) throw Error(); } catch { next.links = 'Enter a complete http or https URL.'; } }
    setErrors(next);
    ...
    try {
      ...
      if (!response.ok || data?.ok !== true) {
        ...
        setNotice('We couldn’t send your note.\nPlease try again.' + (typeof data?.message === 'string' && data.message !== 'We couldn’t send your note. Please try again.' ? `\n${data.message}` : ''));
      } else { setStatus('sent'); setNotice('Thank you.\nWe received your note.\nWe’ll be in touch if there’s a fit.'); }
    } catch { setStatus('error'); setNotice('We couldn’t send your note.\nPlease try again.'); }
  ...
  return <div className="contact-panel">
    <form id="contact-form" ref={formRef} onSubmit={submit} noValidate aria-label="Start a conversation" aria-busy={status === 'sending'}>
      <div className="contact-panel-grid">
        ...
        <div className="contact-panel-field contact-panel-wide"><label htmlFor="contact-links">Links / attachments URL <span>(optional)</span></label><input id="contact-links" name="links" type="url" inputMode="url" maxLength={2048} value={draft.links} onChange={event => update('links', event.target.value)} disabled={status === 'sending' || status === 'sent'} aria-invalid={Boolean(errors.links)} aria-describedby={errors.links ? 'contact-links-error' : undefined} />{errors.links && <span id="contact-links-error" className="contact-field-error">{errors.links}</span>}</div>
        <div className="contact-panel-field contact-panel-wide"><label htmlFor="contact-message">WHAT SHOULD WE BUILD?</label><textarea id="contact-message" name="message" placeholder="Tell us what you want to build, why now, what exists today, and why this deserves to become a real product." rows={5} minLength={20} maxLength={5000} required value={draft.message} onChange={event => update('message', event.target.value)} disabled={status === 'sending' || status === 'sent'} aria-invalid={Boolean(errors.message)} aria-describedby={errors.message ? 'contact-message-error' : undefined} />{errors.message && <span id="contact-message-error" className="contact-field-error">{errors.message}</span>}</div>
      </div>
