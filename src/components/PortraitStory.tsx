  ...
  return <div className={`portrait-story portrait-story-${variant}`} data-subject="sloane">
    <div className="portrait-story-panels">
      ...
      <section className="portrait-workflow-panel portrait-pack-panel">
        ...
        {variant === 'case-study' && <div className="portrait-platforms">{([{platform:'linkedin',label:'LinkedIn'},{platform:'instagram',label:'Instagram'},{platform:'tiktok',label:'TikTok'},{platform:'facebook',label:'Facebook'},{platform:'x',label:'X'},{platform:'youtube',label:'YouTube'},{platform:'slack',label:'Slack'},{platform:'discord',label:'Discord'}] as const).map(({platform,label})=><span className="portrait-destination" key={platform}><PlatformIconBadge platform={platform} /><span className="portrait-destination-label" aria-hidden="true">{label}</span></span>)}</div>}
      </section>
    </div>
