...
const out=new URL('../../../artifacts/1118-brand-director-rebuild/zoom',import.meta.url).pathname;await mkdir(out,{recursive:true});
...
for(const zoom of [2,4]){
 ...
 const page=await context.newPage();await page.goto('http://127.0.0.1:5193/',{waitUntil:'networkidle'});await page.evaluate(()=>document.fonts.ready);
 for(const route of ['/','/work','/work/portrait','/work/signal']){
  if(route!=='/')await page.goto('http://127.0.0.1:5193'+route,{waitUntil:'networkidle'});
 ...
 await page.getByLabel('Name',{exact:true}).fill('Zoom QA');await page.getByLabel('What are you thinking about?',{exact:true}).fill('Local zoom check.');rows.push({zoom:zoom*100,check:'contact editable',pass:await page.getByLabel('Name',{exact:true}).inputValue()==='Zoom QA'});
 await page.close();
}
