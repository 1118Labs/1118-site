...
const out=new URL('../../../artifacts/1118-premium-brand-pass/zoom',import.meta.url).pathname;await mkdir(out,{recursive:true});
...
for(const zoom of [2,4]){
 ...
 const page=await context.newPage();await page.goto('http://127.0.0.1:5191/',{waitUntil:'networkidle'});await page.evaluate(()=>document.fonts.ready);
 for(const route of ['/','/work','/work/portrait','/work/signal']){
  if(route!=='/')await page.goto('http://127.0.0.1:5191'+route,{waitUntil:'networkidle'});
 ...
 await page.goto('http://127.0.0.1:5191/',{waitUntil:'networkidle'});await page.getByRole('button',{name:'Open navigation menu'}).click();await page.keyboard.press('Escape');await page.getByRole('button',{name:'Open navigation menu'}).waitFor();await page.waitForTimeout(80);
 rows.push({zoom:zoom*100,check:'menu keyboard',pass:await page.getByRole('button',{name:'Open navigation menu'}).evaluate(e=>document.activeElement===e)});
 await page.getByLabel('Name',{exact:true}).fill('Zoom QA');await page.getByLabel('What are you building?',{exact:true}).fill('Local zoom check.');rows.push({zoom:zoom*100,check:'contact editable',pass:await page.getByRole('link',{name:'Open email draft'}).getAttribute('href').then(s=>decodeURIComponent(s).includes('Zoom QA'))});
 await page.close();
}
