...
const out = path.resolve('../../artifacts/1118-premium-brand-pass/qa', process.env.QA_RUN_NAME || 'development');
...
try {
  ...
  const track = page.locator('.re-proof-track');
  const carousel = page.locator('.re-proof');
  const current = () => track.evaluate(e => Number(e.style.getPropertyValue('--re-position')));
  ...
  await page.locator('.re-proof-window').focus();
  await page.keyboard.press('End');
  const last = await page.locator('.re-card-card:not(.is-duplicate)').count() - 1;
  ...
  const mcarousel = mp.locator('.re-proof-window');
  await mcarousel.scrollIntoViewIfNeeded();
  box = await mcarousel.boundingBox();
  const mt = mp.locator('.re-proof-track');
  const mBefore = await mt.evaluate(e => Number(e.style.getPropertyValue('--re-position')));
  await swipe({ x:box.x+box.width*.8,y:box.y+Math.min(box.height*.5,280) }, { x:box.x+box.width*.2,y:box.y+Math.min(box.height*.5,280) });
  const mAfter = await mt.evaluate(e => Number(e.style.getPropertyValue('--re-position')));
  ...
  await reduced.page.locator('.re-proof').scrollIntoViewIfNeeded();
  const rt = reduced.page.locator('.re-proof-track');
  const rBefore = await rt.evaluate(e => e.style.getPropertyValue('--re-position'));
  await wait(5600);
  check('reduced motion stable carousel', await rt.evaluate(e => e.style.getPropertyValue('--re-position')) === rBefore, { before:rBefore, after:await rt.evaluate(e=>e.style.getPropertyValue('--re-position')) });
  check('reduced motion hides autoplay control', await reduced.page.getByRole('button', { name:/Pause reviews|Resume reviews/ }).count() === 0, await reduced.page.locator('.re-proof-status').innerText());
  await reduced.page.getByRole('button', { name:'Show next review' }).click();
  check('reduced motion retains manual controls', await rt.evaluate(e => e.style.getPropertyValue('--re-position')) !== rBefore, await rt.evaluate(e=>e.style.getPropertyValue('--re-position')));
