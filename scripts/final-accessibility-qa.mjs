...
const base = process.env.QA_BASE_URL || 'http://localhost:5188';
const out = path.resolve('artifacts/1118-final-launch-candidate/qa-accessibility', process.env.QA_RUN_NAME || 'development');
...
const report = { base, started: new Date().toISOString(), checks: [], viewports: [], runtime: [], limitations: ['Headless Chrome automation does not replace the requested visible founder review.', '200% zoom below uses CSS zoom as an explicit approximation; 400% reflow is tested at a real 320 CSS-pixel viewport (1280 / 4).', 'CLS is measured during this scripted page-loading and scrolling sample, not field telemetry.'] };
...
try {
  if (process.env.QA_MODE === 'policies') {
    ...
    for(const route of ['/privacy','/terms','/accessibility','/support','/security']) {
      ...
      await auditAxe(page,route.slice(1));
      await page.screenshot({path:path.join(out,route.slice(1)+'.png'),fullPage:true});
    }
    await page.goto(base,{waitUntil:'networkidle'});
    await page.getByLabel('Name',{exact:true}).fill('QA Example');
    await page.getByLabel('Email',{exact:true}).fill('qa@example.test');
    const stageSnapshot=await page.getByRole('combobox').ariaSnapshot(); console.log(stageSnapshot); await page.getByRole('combobox').selectOption('Prototype');
    await page.getByLabel('What are you building?',{exact:true}).fill('A local QA draft.');
    const draft=await page.getByRole('link',{name:'Open email draft'}).getAttribute('href');
    check('contact constructs local mail draft',draft.startsWith('mailto:hello@1118.io?')&&decodeURIComponent(draft).includes('QA Example')&&decodeURIComponent(draft).includes('Prototype'),{draft,stageSnapshot});
    await context.close();
  ...
  for (const [width, height] of [[1600,1000],[1440,900],[1280,800],[1024,768],[768,1024],[430,932],[390,844],[320,720]]) {
    ...
    const metrics = await page.evaluate(() => {
      const overflow = Array.from(document.querySelectorAll('body *')).filter(e => {
        ...
        if (s.position === 'absolute' || s.position === 'fixed' || s.visibility === 'hidden' || s.display === 'none' || e.closest('.reviews-proof-window, .portrait-comparison')) return false;
      ...
      const relevantImages = Array.from(document.images).filter(i => {
        ...
        const window=i.closest('.reviews-proof-window');
  ...
  const track = page.locator('.reviews-proof-track');
  const carousel = page.locator('.reviews-proof');
  const current = () => track.evaluate(e => Number(e.style.getPropertyValue('--review-position')));
  await carousel.scrollIntoViewIfNeeded();
  await page.getByRole('button', { name: 'Show next review' }).click();
  const afterNext = await current();
  check('carousel next manual', afterNext === 1, afterNext);
  check('manual action pauses autoplay', await page.getByRole('button', { name: 'Resume reviews' }).isVisible(), await page.getByRole('button', { name: 'Resume reviews' }).count());
  await page.locator('.reviews-proof-window').focus();
  await page.keyboard.press('End');
  const last = await page.locator('.review-fixture-card:not(.is-duplicate)').count() - 1;
  check('carousel End', await current() === last, { current: await current(), expected: last });
  await page.keyboard.press('Home');
  check('carousel Home', await current() === 0, await current());
  await page.keyboard.press('ArrowRight');
  check('carousel ArrowRight', await current() === 1, await current());
  await page.getByRole('button', { name: 'Resume reviews' }).click();
  await page.mouse.move(0, 0);
  const focusedAt = await current();
  await wait(5600);
  check('carousel focus pauses', await current() === focusedAt, { before: focusedAt, after: await current() });
  await slider.focus();
  await carousel.scrollIntoViewIfNeeded();
  await page.mouse.move(0, 0);
  const autoplayAt = await current();
  await wait(5600);
  check('carousel autoplay when visible', await current() !== autoplayAt, { before: autoplayAt, after: await current() });
  await carousel.hover();
  const hoverAt = await current();
  await wait(5600);
  check('carousel hover pauses', await current() === hoverAt, { before: hoverAt, after: await current() });
  await page.mouse.move(0, 0);
  await page.evaluate(() => scrollTo({top:0,behavior:'instant'}));
  const offscreenAt = await current();
  await wait(5600);
  check('carousel offscreen pauses', await current() === offscreenAt, { before: offscreenAt, after: await current() });
  await carousel.scrollIntoViewIfNeeded();
  await page.getByRole('button', { name: 'Pause reviews' }).click();
  await page.locator('.contact-email').focus();
  await carousel.scrollIntoViewIfNeeded();
  await page.mouse.move(0, 0);
  const pausedAt = await current();
  await wait(5600);
  check('carousel explicit pause', await current() === pausedAt, { before: pausedAt, after: await current() });
  await context.close();
  ...
  const mcarousel = mp.locator('.reviews-proof-window');
  await mcarousel.scrollIntoViewIfNeeded();
  box = await mcarousel.boundingBox();
  const mt = mp.locator('.reviews-proof-track');
  const mBefore = await mt.evaluate(e => Number(e.style.getPropertyValue('--review-position')));
  await swipe({ x:box.x+box.width*.8,y:box.y+Math.min(box.height*.5,280) }, { x:box.x+box.width*.2,y:box.y+Math.min(box.height*.5,280) });
  const mAfter = await mt.evaluate(e => Number(e.style.getPropertyValue('--review-position')));
  check('carousel real touch swipe', mAfter !== mBefore, { before:mBefore, after:mAfter });
  await mobile.context.close();

  const reduced = await pageFor({ width:1440,height:900 }, { reducedMotion:'reduce' });
  await reduced.page.locator('.reviews-proof').scrollIntoViewIfNeeded();
  const rt = reduced.page.locator('.reviews-proof-track');
  const rBefore = await rt.evaluate(e => e.style.getPropertyValue('--review-position'));
  await wait(5600);
  check('reduced motion stable carousel', await rt.evaluate(e => e.style.getPropertyValue('--review-position')) === rBefore, { before:rBefore, after:await rt.evaluate(e=>e.style.getPropertyValue('--review-position')) });
  check('reduced motion hides autoplay control', await reduced.page.getByRole('button', { name:/Pause reviews|Resume reviews/ }).count() === 0, await reduced.page.locator('.reviews-proof-status').innerText());
  await reduced.page.getByRole('button', { name:'Show next review' }).click();
  check('reduced motion retains manual controls', await rt.evaluate(e => e.style.getPropertyValue('--review-position')) !== rBefore, await rt.evaluate(e=>e.style.getPropertyValue('--review-position')));
  await reduced.page.screenshot({path:path.join(out,'reduced-motion-reviews.png')});
  await reduced.context.close();

  const zoom = await pageFor({width:1280,height:800});
  await zoom.page.evaluate(()=>{document.documentElement.style.zoom='2';});
  await loadPage(zoom.page);
  const zoomMetrics = await zoom.page.evaluate(()=>({width:innerWidth,scrollWidth:document.documentElement.scrollWidth,bodyRect:document.body.getBoundingClientRect().width}));
  check('200% CSS zoom approximation overflow',zoomMetrics.scrollWidth<=1281,zoomMetrics);
  await zoom.page.screenshot({path:path.join(out,'zoom-200percent-css-approximation.png'),fullPage:true});
