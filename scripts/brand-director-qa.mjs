...
const out = path.resolve('../../artifacts/1118-brand-director-rebuild/qa', process.env.QA_RUN_NAME || 'development');
...
async function auditAxe(page, label) {
  ...
  const actionable = result.violations.filter(v => !(v.id === 'color-contrast' && v.nodes.every(n => n.target.every(t => t.includes('pi-brand-wordmark')))));
  check(`axe ${label}`, actionable.length === 0, { ...result, manualException: 'Only exact Property Insights logotype cyan is exempt under WCAG 1.4.3. Raw findings retained.' });
}
