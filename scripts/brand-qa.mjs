...
const base = process.env.QA_BASE_URL || 'http://127.0.0.1:5195';
const out = path.resolve('/Users/stevehole/.codex/.chatgpt-projects/g-p-6a5ce6e393548191b397a474e9ca78c2/artifacts/1118-brand-unification/qa', process.env.QA_RUN_NAME || 'development');
...
async function pageFor(viewport, extra = {}) {
  const context = await browser.newContext({ viewport, ...extra, ...(verifiedStorage ? {storageState: verifiedStorage} : {}) });
  const page = await context.newPage();
  if (process.env.QA_ACCESS_FILE && !verifiedStorage) {
    const { url } = JSON.parse(await readFile(process.env.QA_ACCESS_FILE, 'utf8'));
    await page.goto(url, { waitUntil: 'domcontentloaded', timeout:45000 });
    await page.locator('h1').waitFor(); verifiedStorage = await context.storageState();
  }
...
async function loadPage(page) {
  ...
  await page.evaluate(async () => { await Promise.race([Promise.all(Array.from(document.images).map(i => i.decode().catch(() => {}))),new Promise(resolve=>setTimeout(resolve,15000))]); });
...
async function auditAxe(page, label) {
  ...
  const actionable = result.violations.filter(v => !(v.id === 'color-contrast' && v.nodes.every(n => n.target.every(t => (t.includes('pi-brand-wordmark') || t === '.fleet-showcase-name > span')))));
  check(`axe ${label}`, actionable.length === 0, { ...result, manualException: 'Exact Property Insights cyan and Reviews Engine gold wordmarks are exempt under WCAG 1.4.3. Raw findings retained.' });
}
...
try {
  if (process.env.QA_MODE === 'policies') {
    ...
    await page.getByLabel('Stage',{exact:true}).selectOption('Prototype');
    await page.getByLabel('What are you building?',{exact:true}).fill('An isolated QA check of the form fields.');
  ...
  for (const [width, height] of [[2560,1440],[1440,900],[1180,820],[768,1024],[390,844],[320,720]]) {
  ...
  const hero = page.getByRole('button', { name: 'Laurie V. Portrait comparison' });
  await hero.focus();
  check('Portrait native focus reveals 56%', await hero.getAttribute('data-etchr-reveal-percent') === '56', await hero.getAttribute('data-etchr-reveal-percent'));
  await hero.press('ArrowLeft'); check('Portrait native ArrowLeft step 6', await hero.getAttribute('data-etchr-reveal-percent') === '50', {});
  await hero.press('ArrowRight'); check('Portrait native ArrowRight step 6', await hero.getAttribute('data-etchr-reveal-percent') === '56', {});
  const heroBox = await hero.boundingBox();
  await page.mouse.move(heroBox.x+heroBox.width*.25,heroBox.y+heroBox.height*.45);await page.mouse.down();await page.mouse.move(heroBox.x+heroBox.width*.75,heroBox.y+heroBox.height*.45,{steps:12});await page.mouse.up();await page.mouse.move(1,1);
  check('Portrait native manual drag persists', Math.abs(Number(await hero.getAttribute('data-etchr-reveal-percent'))-75)<=1 && await hero.getAttribute('data-etchr-manual-reveal')==='true', {});
  const slider = page.getByRole('slider', { name: 'Sloane Portrait comparison' });
  await slider.focus();
  ...
  await page.locator('#portrait .portrait-comparison').screenshot({ path: path.join(out, 'portrait-original.png') });
  await slider.press('End');
  await page.locator('#portrait .portrait-comparison').screenshot({ path: path.join(out, 'portrait-result.png') });
  await slider.press('Home');
  for (let i = 0; i < 25; i++) await slider.press('ArrowRight');
  await page.locator('#portrait .portrait-comparison').screenshot({ path: path.join(out, 'portrait-50percent-focus.png') });
  const stage = await page.locator('#portrait .portrait-comparison').boundingBox();
  ...
  const touchValue = Number(await mp.getByRole('button', { name: 'Laurie V. Portrait comparison' }).getAttribute('data-etchr-reveal-percent'));
  ...
  await mp.locator('.re-proof-window').evaluate(e=>scrollTo({top:e.getBoundingClientRect().top+scrollY-100,behavior:'instant'}));await wait(700);
  const touchReviewBefore=await mp.locator('.re-proof-window').evaluate(e=>e.scrollLeft);
  await swipe({x:330,y:300},{x:80,y:300});
  check('Reviews real touch swipe',await mp.locator('.re-proof-window').evaluate(e=>e.scrollLeft)>touchReviewBefore+100,{});
  check('Reviews touch pauses motion',await mp.getByRole('button',{name:'Resume reviews'}).count()===1,{});
  await mobile.context.close();

  const reduced = await pageFor({ width:1440,height:900 }, { reducedMotion:'reduce' });
  const reducedHero=reduced.page.locator('.portrait-hero-reveal');await reducedHero.focus();
  check('Portrait native reduced motion',await reducedHero.locator('.portrait-hero-result').evaluate(e=>parseFloat(getComputedStyle(e).transitionDuration)===0),{});
  ...
  report.fatal = { message:error.message.replace(/_vercel_share=[A-Za-z0-9]+/g,'_vercel_share=[REDACTED]'), stack:error.stack?.replace(/_vercel_share=[A-Za-z0-9]+/g,'_vercel_share=[REDACTED]') };
